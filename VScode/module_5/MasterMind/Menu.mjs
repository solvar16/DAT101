...
export class TMenu{
    ...
    constructor(aSpriteCanvas){
        ...
        this.#buttonNewGame =
        new libSprite.TSpriteButtonHaptic(aSpriteCanvas, SpriteInfoList.ButtonNewGame, MastermindBoard.ButtonNewGame);
        this.#buttonCheckAnswer =
        new libSprite.TSpriteButtonHaptic(aSpriteCanvas, SpriteInfoList.ButtonCheckAnswer, MastermindBoard.ButtonCheckAnswer);
        this.#buttonCheat =
        new libSprite.TSpriteButtonHaptic(aSpriteCanvas, SpriteInfoList.ButtonCheat, MastermindBoard.ButtonCheat);
        this.#panelCheat =
    ...
    onCheckAnswerClick = () => {
        ...
        for(let i = 0; i < 4; i++){
            ...
            if(computerAnswer.color === playerAnswer.color){
                ...
                //Er alle fargene riktig når spillet er over, og vi må vise fargene fra computeren
                ...
                for(let j = 0; j < 4; j++){
                    ...
                    if(computerAnswer.checkThis && (playerAnswer.pos !== computerAnswer.pos)){
                        if(playerAnswer.color === computerAnswer.color){
                            console.log(`Rett farge på feil plass - ${playerAnswer.pos + 1}, ${computerAnswer.pos + 1}`)//`tekst men at vi også kan putte inn variabler i teksten.
    ...
    //hvorfor er denne private? den gjemmes. fordi variblene bruker interne variabler, som ikke en forekomst vet hva er for noe?
